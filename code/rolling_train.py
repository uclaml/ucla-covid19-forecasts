...
def train(model, init, prev_params, train_data, reg=0, lag=0):
    ...
    def loss_train(params):
        ...
        + 1*loss(pred_ave_confirm_perday, target_ave_confirm_perday) + 1 * \
    ...
    optimal = minimize(
        ...
        method='L-BFGS-B',
        bounds=[(0.0001, .3), (0.001, 0.3), (0.01, 1), (0.0001, 1.)]
    )
...
def rolling_prediction(model, init, params_all, train_data, new_sus, pred_range, pop_in=1/500, daily_smooth=False):
    ...
    modified_slope_gap_confirm = np.minimum(modified_slope_gap_confirm, ave_confirm_perday/40)
    ...
    temp_F_perday = np.diff(pred_fatality)
