...
parser.add_argument('--county', default = "default",
                    help='county')
parser.add_argument('--pred_type', default = "prediction",
                    help='starting date for predictions')
args = parser.parse_args()
PRED_START_DATE = args.END_DATE if args.pred_type == "validation" else args.VAL_END_DATE
...
def write_csv_files(counties, write_csv_files, pred_type):

    write_dir = "predict/NYC"
    if pred_type == "validation":
        write_dir = "test/NYC"

    result_test = pd.concat(write_csv_dict["upper"])
    result_test.insert(0, "", counties)
    result_test.to_csv("pred_results_county/" + write_dir + "us_county_cases_prediction_upper_bound.csv", index=False)

    result_test = pd.concat(write_csv_dict["lower"])
    result_test.insert(0, "", counties)
    result_test.to_csv("pred_results_county/" + write_dir + "us_county_cases_prediction_lower_bound.csv", index=False)

    result_test = pd.concat(write_csv_dict["true"])
    result_test.insert(0, "", counties)
    result_test.to_csv("pred_results_county/" + write_dir + "us_county_cases_prediction.csv", index=False)
...
def get_county_list(cc_limit=200, pop_limit=50000):
    ...
    for region in County_Pop.keys():
        ...
        # print(county,state)
        # print(state)
        if County_Pop[region][0]>=pop_limit and not state in non_county_list and not county == "Waukesha":        
            ...
            if len(death) >0 and np.max(death)>=0 and np.max(confirm)>cc_limit and start_date < "2020-05-01":
                county_list += [region]
...
if not args.state == "default" and not args.county == "default":
    ...
    val_dir = "val_results_county/test" + args.dataset + "_"

else:
    # region_list = get_county_list(cc_limit=1000, pop_limit=5000)
    # print("# feasible counties:", len(region_list))
    val_dir = "val_results_county/" + args.dataset + "_" 
pred_dir = "pred_results_county/"
# val_dir = "val_results_county/" 


json_file_name = val_dir  + "val_params_best_END_DATE_" + args.END_DATE + "_VAL_END_DATE_" + args.VAL_END_DATE
...
counties = []
write_csv_dict = {}
write_csv_dict["upper"], write_csv_dict["true"], write_csv_dict["lower"] = [],[],[]
...
for region in region_list:
    ...
    if start_date < "2020-05-10":
        ...
        full_data = [data.get(start_date, PRED_START_DATE, state, county)]
    ...
    if len(train_data)>1:
        ...
        if args.level=="state" and reopen_flag and (np.mean(daily_confirm[-7:])<12.5 or mean_increase<1.1):
            pop_in = 1/500
        if args.level == "nation" and (region == "France" or region == "Spain" or region == "Germany" or region == "Italy" or region=="Canada"):
            pop_in = 1/5000
    ...
    loss_true = rolling_likelihood(model, init, params_all, train_data, new_sus, pop_in=pop_in)[-1]
        ...
        for beta0 in beta_list:
            for gamma0 in gamma_list:
                for sigma0 in sigma_list:
                    for mu0 in mu_list:
                        temp_param = [params_all[0]] + [np.asarray([beta0,gamma0,sigma0,mu0])] if len(params_all)>1 else [np.asarray([beta0,gamma0,sigma0,mu0])]
                        ...
                        loss = rolling_likelihood(model, init, temp_param, train_data, new_sus, pop_in=pop_in)[-1]
    ...
    predict_range = 7 if args.pred_type == "validation" else 30
    ind = dates.index(datetime.strptime(PRED_START_DATE,"%Y-%m-%d"))+1
    dates = [dates[i].strftime('%-m/%-d/%y') for i in range(len(dates)) if i>=ind and i<ind+predict_range]
    pred_data_upper = pd.DataFrame(data=[maxI[ind:ind+predict_range]], columns=dates)
    pred_data_true = pd.DataFrame(data=[meanI[ind:ind+predict_range]], columns=dates)
    pred_data_lower = pd.DataFrame(data=[minI[ind:ind+predict_range]], columns=dates)

    write_csv_dict["upper"] += [pred_data_upper]
    write_csv_dict["true"] += [pred_data_true]
    write_csv_dict["lower"] += [pred_data_lower]

    write_csv_files(counties, write_csv_dict, args.pred_type)
