...
def get_county_list(cc_limit=200, pop_limit=50000):
    ...
    for region in County_Pop.keys():
        ...
        if County_Pop[region][0]>=pop_limit and not state in non_county_list:        
            ...
            if len(death) >0 and np.max(death)>=0 and np.max(confirm)>cc_limit and start_date < "2020-07-04":
                county_list += [region]
...
write_file_name = pred_start_date+ "-UCLA-SuEIR_county.csv"
...
json_file_name = "val_results_county/" + "JHU" + "_val_params_best_END_DATE_" + args.END_DATE + "_VAL_END_DATE_" + args.VAL_END_DATE
with open(json_file_name, 'r') as f:           
    NE0_region = json.load(f)

# region_list = get_county_list(cc_limit=10, pop_limit=10)
# region_list = ["Fayette_Alabama"]
...
for region in region_list:
    ...
    if len(train_data)>1:
        ...
        if np.mean(daily_confirm[-7:])<12.5 or mean_increase<1.1:
            ...
            pop_in = 1/1000
    # print (pop_in)
    print("region: ", region, " start date: ", start_date, " training end date: ", args.END_DATE, " prediction start date: ", PRED_START_DATE, " mid date: ", second_start_date)  
    ...
    confirm = full_data[0][0][0:-1].tolist() + full_data[1][0][0:-1].tolist() + pred_true[0].tolist() if len(train_data)>1 else full_data[0][0][0:-1].tolist() + pred_true[0].tolist()
    ...
    #     # write cumulative deaths
    #     death_lastsat = full_data[-1][1][-2]
    #     diffR_wk = np.zeros(R_inv_wk.shape)
    #     diffR_wk[:, 1:] = np.maximum(0, np.diff(R_inv_wk))
    #     diffR_wk[:, 0] = np.maximum(0, R_inv_wk[:,0]-death_lastsat)
